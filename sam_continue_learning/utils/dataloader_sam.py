...
class SamDataset(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False, batch_size_prompt=-1, batch_size_prompt_start=0, mode='train'):
        ...
        self.adv_prompt_training = True if 'adv_boxes_path' in name_im_gt_list[0].keys() else False
        self.gt_noisy_prompt = True if 'gt_noisy_prompt' in name_im_gt_list[0].keys() else False
        self.boxes_noise_scale = name_im_gt_list[0]['boxes_noise_scale'] if 'gt_noisy_prompt' in name_im_gt_list[0].keys() else 0.
    ...
    def __getitem__(self, idx):
        ...
        if self.gt_noisy_prompt: 
            sample['boxes_noise_scale'] = self.boxes_noise_scale
            sample['gt_noisy_prompt'] = True
        ...
        if self.adv_prompt_training:
            ...
            if os.path.exists(adv_points_path):
                adv_points = torch.tensor(ast.literal_eval(open(adv_points_path,'r').read())) # [N 10 2]
            else: 
                adv_points = misc.masks_sample_points(adv_prompt_sample['label'], k=10) # [N 10 2]
            ...
            adv_prompt_sample['adv_prompt_training'], adv_prompt_sample['adv_boxes'], adv_prompt_sample['adv_points'] = True, adv_boxes, adv_points
        else:
            adv_prompt_sample = {}
            adv_prompt_sample['image'], adv_prompt_sample['label'] =\
            torch.squeeze(F.interpolate(torch.unsqueeze(im,0),(1024,1024),mode='bilinear'),dim=0),\
