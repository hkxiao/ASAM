...
def check_control_net(pipe):
    ...
    output = pipe(
    "a stairway leading up to a building with ivy growing on it",  image=control_image,num_inference_steps=50,controlnet_conditioning_scale=4.0
    ).images[0]
    
    output_numpy = np.array(output)
    print(type(output),output_numpy.max())
    output.save('demo_few.png')
...
if __name__ == '__main__':
    ...
    controlnet = ControlNetModel.from_single_file("ckpt/control_v11p_sd15_mask_sa000000.pth").to(device)     
   # controlnet = ControlNetModel.from_single_file("ckpt/control_v11p_sd15_mask_sa000001.pth").to(device)  
   # controlnet = ControlNetModel().to(device)                    
    ...
    # Batch Images Load
    dataset_dir = '/data/tanglv/data/sam-1b/sa_000000'
    
    all_latents = torch.empty(0, 4, 64, 64)
    all_uncons = torch.empty(0, NUM_DDIM_STEPS, 77, 768)
    ...
    save_path = '11187-Inversion/'
    if not os.path.exists(save_path): os.mkdir(save_path)
    if not os.path.exists(save_path + 'pair'): os.mkdir(save_path + 'pair')
    if not os.path.exists(save_path + 'embeddings'): os.mkdir(save_path + 'embeddings')
    
    for i in trange(args.start, args.end+1):
        img_path = os.path.join(dataset_dir, 'sa_'+ str(i) + '.jpg')
        mask_path = os.path.join(dataset_dir, 'sa_'+ str(i) + '.png')
        
        if not os.path.exists(img_path):
            print(img_path, "does not exist!")
            continue
        
        latent_path = f"{save_path}/embeddings/sa_{i}_latent.pth"
        uncond_path = f"{save_path}/embeddings/sa_{i}_uncond.pth"
        ...
        mask_control = cv2.imread(mask_path).astype(np.float32)
        ...
        start = time.time()
        (image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(img_path, mask_control=mask_control, prompt = prompt, offsets=(0,0,0,0), verbose=True, num_inner_steps=args.steps)
        ...
        image_inv, x_t = run_and_display(prompts=[""], controller=controller, run_baseline=False, latent=x_t, mask_control=mask_control,uncond_embeddings=uncond_embeddings, verbose=False)
        
        ptp_utils.view_images([image_gt, image_inv[0]], prefix='debug')
        raise NameError