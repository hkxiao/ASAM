...
def text2image_ldm_stable_last(
    ...
    for k in range(args.steps):
        ...
        with torch.enable_grad():
            ...
            gather_ad_low_res_logits = torch.empty(0,1,256,256).cuda() 
            gather_loss_ce = 0
            ...
            for i, annotation in enumerate(annotations):
                ...
                if i==len(annotations)-1 or (i+1)%batch_size==0:
                    start, end = i // batch_size * batch_size, i+1                   
                    ad_masks, ad_iou_predictions, ad_low_res_logits = net.predict_torch(point_coords=None,point_labels=None,boxes=boxes[start:end],multimask_output=False)
                    gather_ad_low_res_logits = torch.cat([gather_ad_low_res_logits,ad_low_res_logits.detach()])
                    loss_ce = torch.nn.functional.binary_cross_entropy_with_logits(ad_low_res_logits,mask_labels[start:end]) 
                    scale = end - start
                    loss_ce = loss_ce * scale
                    gather_loss_ce += loss_ce.item()
                    loss_ce.backward() 
                
                if i == len(annotations)-1:
                    iou = compute_iou(gather_ad_low_res_logits, mask_labels.detach()).item()
                    if iou < worst_iou:
            ...
            all_loss = gather_loss_ce + loss_mse.item()
            # print(image.shape)
            # print(latents.shape, latent.shape)
            print('*' * 50)
            print('Loss', all_loss,'Loss_ce'. gather_loss_ce, 'Loss_mse', loss_mse.item())
            print(k, 'Predicted:', all_loss)
...
if __name__ == '__main__':
    ...
    save_path = './11187-Grad/' + args.prefix + '-' + str(args.mu) + '-' + args.model + '-' + str(args.alpha) + '-' + str(args.beta) + '-' + str(args.norm) + '-' + str(args.steps) + '-Clip-' + str(args.eps) + '/'
