...
parser.add_argument('--mask_control_scale', type=float, default=1.0)
parser.add_argument('--boundary_control_scale', type=float, default=0.5)
...
parser.add_argument('--boundary_controlnet_path', default='ckpt/control_v11p_sd15_mask_sa000000.pth', type=str, help='random seed')    
...
def text2image_ldm_stable_control(
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
        ...
        latents = diffusion_step_control(model, controller, control_mask, control_scale, latents, context, t, guidance_scale, pooled_context, add_time_ids,low_resource=False,guess_mode=args.guess_mode)
...
def get_noise_pred_control(model, control_mask,control_scale,latents, t, context, pooled_context, add_time_ids):    
    ...
    down_block_res_samples_boundary, mid_block_res_sample_boundary = boundary_controlnet(
            latents,
            t,
            encoder_hidden_states=context,
            controlnet_cond=control_boundary,
            conditioning_scale=args.boundary_control_scale,
            added_cond_kwargs=added_cond_kwargs,
            return_dict=False,
    )
...
if __name__ == '__main__':
    ...
    mask_controlnet = ControlNetModel.from_pretrained(args.mask_controlnet_path).to(device)    
    boundary_controlnet = ControlNetModel.from_pretrained(args.boundary_controlnet_path).to(device) 
    ...
    save_path = save_path + '-Control-' + str(args.mask_control_scale) + '-' + str(args.boundary_control_scale)
    ...
    for i in trange(args.start, args.end+1):
        ...
        control_boundary_path = args.control_mask_dir+'/'+'sa_'+str(i)+'_canny.png'
        ...
        control_boundary = cv2.imread(control_boundary_path)
        control_boundary = cv2.cvtColor(control_boundary, cv2.COLOR_BGR2RGB)
        control_boundary= cv2.resize(control_boundary, (1024,1024))
        boundary_show = control_boundary.copy()
        control_boundary = torch.from_numpy(control_boundary).permute(2,0,1).unsqueeze(0).to(torch.float32).cuda() / 255.0
        
        # show 
        controller = EmptyControl()
        image_control, x_t = run_and_display_control(prompts=[prompt], controller=controller, run_baseline=False, latent=x_t, control_mask=control_mask, control_scale=args.mask_control_scale, uncond_embeddings=uncond_embeddings, verbose=False)
        ptp_utils.view_images([img,inv_img,grad_img, image_control[0],mask_show,boundary_show], prefix=f'{save_path}/pair/sa_{i}', shuffix='.jpg')
