...
def main(args):
    ...
    with distributed_state.main_process_first():
        ...
        if args.model == 'controlnet':
            ...
            if '.pth' in args.model_path: 
                controlnet = ControlNetModel.from_single_file(args.model_path).cuda().to(torch.float16)
            else: 
    ...
    with distributed_state.main_process_first():
        if args.task_name == 'depth':
            ...
            from utils import get_reward_model
            model = get_reward_model(task='interactive_segmentation', model_path='https://huggingface.co/lllyasviel/Annotators/resolve/main/ControlNetHED.pth')
            model.eval()
    ...
    with distributed_state.split_between_processes(list(range(len(dataset)))) as local_idxs:
        ...
        for idx in local_idxs:
            ...
            if args.task_name == 'interactive_segmentation': label = condition
            ...
            [img.save(f"{save_dir}/images/group_{i}/{uid}.jpg") for i, img in enumerate(images)]
            ...
            show(images)
            plt.tight_layout()
            plt.savefig(f'{save_dir}/visualization/{uid}.jpg', bbox_inches='tight', dpi=512)
            plt.clf()
