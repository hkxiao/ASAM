...
import sys
sys.path.append('../sam_continue_learning')
...
def get_reward_model(task='segmentation', model_path='mmseg::upernet/upernet_r50_4xb4-160k_ade20k-512x512.py'):
    ...
    elif task == 'interactive_segmentation':
        return build_efficient_sam_vitt(checkpoint="../sam_continue_learning/pretrained_checkpoint/efficient_sam_vitt.pt")
    elif task == 'canny':
        return Canny()
    elif task == 'depth':
...
def get_reward_loss(predictions, labels, task='segmentation', **args):
    ...
    """
    if task == 'interactive_segmentation':
        loss_ce = torch.nn.functional.binary_cross_entropy_with_logits(predictions, labels/255.0, **args).mean((-1,-2,-3))
        loss_dice =  dice_loss(predictions.sigmoid(), labels/255.0)   
...
def seg_label_transform(
    ...
    if dataset_name == 'limingcv/Captioned_ADE20K':
        ...
        labels = F.resize(labels, output_size, max_size=max_size, antialias=antialias)
