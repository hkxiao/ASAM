...
def parse_args(input_args=None):
    ...
    parser.add_argument("--batch_size_prompt", type=int, default=4)
    parser.add_argument("--batch_size_prompt_start", type=int, default=0)
...
def make_train_dataset(args, tokenizer, accelerator, split='train'):
    ...
    def preprocess_train(examples):
        ...
        if args.label_column == 'label_dir':
            label_num = len(os.listdir(examples[args.label_column][0]))
            label_dir =  examples[args.label_column][0]
            
            if args.batch_size_prompt == -1: end = label_num
            else: end = args.batch_size_prompt_start+args.batch_size_prompt
            
            labels = None
            for i in range(args.batch_size_prompt_start, end):
                if i >= label_num: i = (i - args.batch_size_prompt_start) % (label_num - args.batch_size_prompt_start) + args.batch_size_prompt_start
                
                tmp_label = io.imread(os.path.join(label_dir,'segmentation_'+str(i)+'.png'))
                if len(tmp_label.shape) > 2:
                    tmp_label = tmp_label[:, :, 0]
                tmp_label = np.expand_dims(tmp_label, axis=0)

                if type(labels)!= np.ndarray: labels = tmp_label
                else: labels = np.concatenate([labels, tmp_label])
            labels = [F.interpolate(torch.tensor(labels).to(torch.float32).unsqueeze(0),size=resolution,mode='bilinear',align_corners=False).squeeze(0)]
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        if '.pth' in args.controlnet_model_name_or_path: 
            controlnet = ControlNetModel.from_single_file(args.controlnet_model_name_or_path).cuda()
        else: controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
    ...
    if args.use_ema:
        if '.pth' in args.controlnet_model_name_or_path: ema_controlnet = ControlNetModel.from_single_file(args.controlnet_model_name_or_path).cuda()
        else: ema_controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
    ...
    reward_weight_dtype = torch.float32

    # Move vae, unet and text_encoder to device and cast to weight_dtype
    vae.to(accelerator.device, dtype=weight_dtype)
    unet.to(accelerator.device, dtype=weight_dtype)
    text_encoder.to(accelerator.device, dtype=weight_dtype)
    reward_model.to(accelerator.device, dtype=reward_weight_dtype)
    reward_model.eval()
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            print(path, args.resume_from_checkpoint)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                elif args.task_name in ['canny', 'lineart', 'hed', 'interactive_segmentation']:
                    pass
                else:
                    image = normalize(image, (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))

                # reward model inference
                if args.task_name == 'interactive_segmentation':
                    image_m = F.interpolate(image, (1024,1024))

                    B, N = batch['labels'].shape[:2]
                    masks_1024 = F.interpolate(batch['labels'], (1024, 1024), align_corners=False, mode='bilinear').view(B*N,1024,1024)
                    
                    boxes = masks_to_boxes(masks_1024)
                    input_points = boxes.reshape(B,N,2,2)
                    input_labels = torch.concat([torch.full((B,N,1),2), torch.full((B,N,1),3)] ,-1).cuda()
                    
                    predicted_logits, predicted_iou = reward_model(
                        image_m.to(reward_weight_dtype),
                        input_points.to(reward_weight_dtype),
                        input_labels.to(reward_weight_dtype),
                    )
                    ...
                    outputs = F.interpolate(predicted_logits[:,:,0,:,:],(512,512),mode='bilinear',align_corners=False)
                ...
                if args.task_name == 'interactive_segmentation': pass
                ...
                if args.task_name == 'segmentation':
                    ...
                    pass
                elif args.task_name == 'interactive_segmentation':
                    pass
                else:
