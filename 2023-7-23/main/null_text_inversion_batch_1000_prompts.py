...
class NullInversion:
    ...
    def invert(self, image_path: str, mask_path: str,prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        self.init_prompt(prompt)
        ptp_utils.register_attention_control(self.model, None)
        image_gt = load_512(image_path, *offsets)
        mask_control = load_512(mask_path, *offsets).astype(np.float32) / 255.0
        mask_control = torch.from_numpy(mask_control).permute(2,0,1)
        mask_control = mask_control[None,...].cuda()
...
def text2image_ldm_stable(
    ...
    mask_control = load_512(mask_path).astype(np.float32) / 255.0
    mask_control = torch.from_numpy(mask_control).permute(2,0,1)
    mask_control = mask_control[None,...].cuda()
...
def run_and_display(prompts, controller, latent=None, mask_path=None, run_baseline=False, generator=None, uncond_embeddings=None, verbose=True, prefix='inversion'):
    if run_baseline:
        print("w.o. prompt-to-prompt")
        images, latent = run_and_display(prompts, EmptyControl(), latent=latent, mask_path=mask_path, run_baseline=False, generator=generator)
        print("with prompt-to-prompt")
    images, x_t = text2image_ldm_stable(ldm_stable, prompts, controller, latent=latent, mask_path=mask_path,num_inference_steps=NUM_DDIM_STEPS, guidance_scale=GUIDANCE_SCALE, generator=generator, uncond_embeddings=uncond_embeddings)
    if verbose:
...
if __name__ == '__main__':
    '''
        python null_text_inversion_batch_1000_prompts.py CUDA_VISBLE_DEVICES=1
    '''
    ...
    img_filepath = '/data/tanglv/data/sod_data/DUTS-TR/imgs'
    mask_filepath = '/data/tanglv/data/sod_data/DUTS-TR/gts'
    filepath_list = sorted(os.listdir(img_filepath))
    avg_ssim, avg_mse, avg_psnr = 0, 0, 0
    
    for idx in trange(image_nums):
        ...
        #prompts = model.generate({"image": image, "prompt": "Question: Please give a detailed description of the image. Answer:"})
        prompts = [info['prompt'].strip()]
        print(prompts)

        start = time.time()
        # Image Inversion
        (image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(img_path, mask_path,prompts[0], offsets=(0,0,0,0), verbose=True)
        ...
        image_inv, x_t = run_and_display(prompts, controller, run_baseline=False, latent=x_t, mask_path=mask_path,uncond_embeddings=uncond_embeddings, verbose=False)
