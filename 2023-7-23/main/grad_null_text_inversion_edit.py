...
parser.add_argument('--ddim_step', default=50, type=int, help='random seed')
...
def text2image_ldm_stable_last(
    ...
    for k in range(args.steps):
        ...
        with torch.enable_grad():
            ...
            example['box'] = input_box
            example['original_size'] = (1024,1024)
            outputs = net([example], multimask_output=False)[0]['low_res_logits']

            
            mae = mae_value(mask_label, outputs)
            if mae >= args.mae_thd:
                success = False
                if mae > worst_mae:
                    best_latent, worst_mae  = adv_latents, mae
                
            image_m = image_m - mean[None,:,None,None]
            image_m = image_m / std[None,:,None,None]
            print(outputs.shape)
            
            loss_ce = torch.nn.functional.binary_cross_entropy_with_logits(outputs,mask_label)
...
if __name__ == '__main__':
    ...
    all_info = open(dataset_dir + '/blip2_prompt.json').readlines()
    all_box_info = open(dataset_dir + '/box.json').readlines()
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
        os.mkdir(save_path+'pair/')
        os.mkdir(save_path+'adv/')
     
    import json    
    for idx in trange(args.start, args.end):
        info = json.loads(all_info[idx])
        img_path = os.path.join(dataset_dir, info['target'])
        
        # if os.path.exists(os.path.join(save_path, 'adv', img_path.split('.')[0]+'.png')):
        #     print(os.path.join(save_path, img_path.split('.')[0]), " has existed!")
        
        mask_path = os.path.join(dataset_dir, info['source'])
        ...
        raw_img_numopy = np.array(pil_image.resize((512,512)))
        ...
        pil_mask = Image.open(mask_path).resize((256,256)).convert('L')
        mask_label = torch.tensor(np.array(pil_mask).astype(np.float32), device=device).unsqueeze(0).unsqueeze(0)/255.
        # mask_label = (mask_label >= 0.5).to(torch.float32)
        
        box_info = json.loads(all_box_info[idx])
        input_box =  torch.tensor(box_info['box'], device=device, dtype=torch.float32)
        
        pil_mask = Image.open(mask_path).resize((512,512))
        mask_numpy= np.array(pil_mask)
        mask_control= (torch.tensor(np.array(pil_mask).astype(np.float32), device=device).unsqueeze(0)/255.).permute(0, 3, 1, 2)    
        
        prompts = [prompt]
        controller = EmptyControl()
        image_inv, x_t, success = text2image_ldm_stable_last(ldm_stable, prompts, controller, latent=x_t, num_inference_steps=NUM_DDIM_STEPS, guidance_scale=GUIDANCE_SCALE, generator=None, uncond_embeddings=uncond_embeddings, mask_label=mask_label, mask_control=mask_control,raw_img=raw_img,input_box=input_box)
        
        ptp_utils.view_images([image_inv[0]], prefix=os.path.join(save_path, 'adv',info['target'].split('/')[-1].split('.')[0]))
        ptp_utils.view_images([cv2.resize(image_inv[0],(512,512)), raw_img_numopy, mask_numpy], prefix=os.path.join(save_path, 'pair',info['target'].split('/')[-1].split('.')[0]))
        cnt += success
        
        print("Acc: ", cnt, '/', (idx-args.start+1))