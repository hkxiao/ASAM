...
if __name__ == '__main__':
    '''
        python null_text_inversion_batch_11187_prompts.py CUDA_VISBLE_DEVICES=0
    '''    
    ...
    controlnet = ControlNetModel.from_single_file("pretrained/control_v11p_sd15_mask_sam.pth").to(device)    
    ...
    if not os.path.exists(save_path+'/inversion/'): os.mkdir(save_path+'/inversion/')
    if not os.path.exists(save_path+'/original/'): os.mkdir(save_path+'/original/')
    ...
    for i in trange(args.start, args.end+1):
        ...
        info_dict = json.loads(open(json_path,'r').read())
        ...
        for j, annotation in enumerate(annotations):
            ...
            rle_encoded_mask = info_dict['annotations'][j]['segmentation']
            ...
            start = time.time()
            # Image Inversion
            #(image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(img_path, mask_control=mask_control, prompt = prompt, offsets=(0,0,0,0), verbose=True)
            ...
            #torch.save(x_t, f'temp/11187/sa_{i}_{j}_latent.pth')
            #torch.save(gather_uncond_embeddings, f'temp/11187/sa_{i}_{j}_uncond.pth')
            ...
            print(prompt,mask_control.shape,mask_control.min(),mask_control.max())
            # raise NameError
            ...
            ptp_utils.view_images([image_inv[0]], prefix=os.path.join('sa_'+str(i)+'_'+str(j)))
            
            #ptp_utils.view_images([image_gt, image_inv[0]], prefix=f'11187/pair/sa_{i}_{j}')
            #ptp_utils.view_images([image_gt], prefix=f'11187/original/sa_{i}_{j}')
            #ptp_utils.view_images([image_inv[0]], prefix=f'11187/inversion/sa_{i}_{j}')
